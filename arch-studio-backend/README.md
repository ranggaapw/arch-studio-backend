...
_Membangun jembatan antara konten dinamis dan tampilan website yang profesional_

> **Catatan struktur repo:** source code Spring Boot dan Gradle berada di folder `arch-studio-backend/` di dalam repository ini. Saat menjalankan perintah build atau membuka project di editor, gunakan folder tersebut sebagai root aplikasi.
...
**Arch Studio Backend** adalah RESTful API yang dibangun menggunakan **Java 25** dan **Spring Boot 4.1.0** untuk mendukung sistem _company profile_ studio arsitektur dan desain interior **MDK (Mitra Daya Kreasi)**. API ini melayani seluruh kebutuhan data dinamis halaman website, mulai dari manajemen portofolio proyek, lowongan kerja, hingga pengaturan konten halaman oleh admin.

### ✨ Tujuan

Memungkinkan tim admin untuk mengelola seluruh konten website secara _real-time_ melalui dashboard admin tanpa perlu menyentuh kode sama sekali, dengan data yang tersimpan permanen di database PostgreSQL.
...
│   │   │       └── WebConfiguration.java           # Konfigurasi CORS & pendaftaran interceptor
│   │   │
│   │   └── resources/
...
- Akun admin awal dibuat otomatis oleh seeder jika tabel `users` masih kosong.
- Kredensial default seeder: `admin / admin123`.
...
> Modul proyek saat ini menyimpan metadata berikut: judul, deskripsi, image utama, kategori, client name, location, year, materials, gallery images, budget, dan flag rekomendasi.
...
cd arch-studio-backend/arch-studio-backend
```
...
Validasi build yang sudah dicek: `./gradlew compileJava` berhasil dijalankan dari root project yang benar.
...
Pengaturan konten seluruh halaman website (Hero Banner, Tentang Kami, Layanan, Karir) dapat diubah secara _real-time_ melalui endpoint admin tanpa perlu deploy ulang atau menyentuh kode.
